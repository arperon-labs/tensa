# License section — paste into your main README.md

Drop the following block into your `README.md`, typically near the bottom
just above the acknowledgements.

---

## License

TENSA is **dual-licensed**. Choose the license that fits your use case:

- **[GNU AGPL-3.0](./LICENSE)** — free, for research, open-source
  projects, and internal use. Requires that you release the source of
  any product that embeds or network-serves TENSA under AGPL-3.0 too.
- **Commercial license** from Arperon s.r.o. — for proprietary
  products, SaaS, and any use that cannot accept AGPL-3.0 obligations.
  See [`COMMERCIAL-LICENSE-TERMS.md`](./COMMERCIAL-LICENSE-TERMS.md) or
  email **licensing@arperon.com**.

See [`DUAL-LICENSING.md`](./DUAL-LICENSING.md) for how to choose and a
plain-English FAQ.

Contributions are accepted under the [Contributor License Agreement](./CLA.md).

Copyright © 2026 Arperon s.r.o. TENSA is a trademark of Arperon s.r.o.
